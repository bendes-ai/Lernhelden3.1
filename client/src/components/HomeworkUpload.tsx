...
interface OcrData {
  ...
  wordCount: number; processingMs: number;
  method: 'sharp' | 'opencv' | 'raw';
}
interface Task {
  id: string; type: string; title: string;
  instruction: string; difficulty: 'easy' | 'medium' | 'hard';
}
...
interface ApiSuccess {
  success: true;
  ocr: OcrData;
  subject: string; topic: string; emoji: string; reason: string;
  tasks: Task[];
  flashcards: Flashcard[];
}
interface ApiError { success: false; error: string; message: string; }
...
function ErrorBox({ msg }: { msg: string }) {
  return <div className="error-box" style={{ display:'flex', gap:'.5rem', alignItems:'flex-start' }}><span>❌</span><span>{msg}</span></div>;
}
...
export default function HomeworkUpload() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/homework/upload', { method:'POST', body: fd });
      setStage('🤖 Mistral analysiert Fach und erstellt Aufgaben …');
      const data: ApiResponse = await r.json();
      if (!r.ok || !data.success) throw new Error((data as ApiError).message || 'Fehler');
    ...
    return (
      <div className="grid" style={{ gap:'1.5rem' }}>
        ...
        <div className="card">
          ...
          {showText && (
            <pre style={{ marginTop:'1rem', background:'#F8F7FF', borderRadius:10, padding:'1rem',
              fontSize:'.82rem', fontFamily:'monospace', whiteSpace:'pre-wrap', maxHeight:240, overflowY:'auto' }}>
              {result.ocr.text}
            </pre>
          )}
        </div>
  ...
  return (
    <div className="grid" style={{ gap:'1.25rem' }}>
      {/* Datenschutz-Banner */}
      <div style={{ background:'rgba(255,213,0,.15)', border:'2px solid #FFD700', borderRadius:12, padding:'1rem 1.25rem', display:'flex', gap:'.75rem' }}>
        <span>🔒</span>
        <p style={{ fontSize:'.88rem' }}>
          <strong>Datenschutz:</strong> Bilder werden lokal per Tesseract.js + Sharp + OpenCV verarbeitet und nach der OCR sofort gelöscht. Lade keine Fotos mit persönlichen Daten hoch.
        </p>
      </div>
      ...
      >
        {preview ? (
          <>
            <img src={preview} alt="Vorschau" style={{ maxWidth:'100%', maxHeight:200, borderRadius:10, marginBottom:'.75rem' }}/>
            <p style={{ fontWeight:700, color:'var(--primary)' }}>✅ {file!.name} ({(file!.size/1024).toFixed(0)} KB)</p>
          </>
        ) : (
          <>
      ...
      <div style={{ display:'flex', gap:'.75rem', flexWrap:'wrap' }}>
        ...
        {file && !loading && <button className="btn btn-outline" onClick={reset}>✕ Reset</button>}
      </div>
    </div>
  );
}
